...
6. HTML 컴포넌트화는 페이지 전체가 아니라 `main` 내부 section 단위로 분리한다
7. 스니펫 확인: `src/snippets/{component}.md` — 기존 패턴 우선 적용
8. BEM Block명은 `src/styles/6-components/{component}.css` 파일명과 일치
9. 인터랙티브 요소: `role`, `aria-*`, `tabindex` 확인
10. 폼 요소: `<label for>` + `id` 연결 필수
11. 모바일 터치 영역 ≥ 44×44px (R-13)
