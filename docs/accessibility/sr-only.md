...
> CSS 유틸리티 코드는 `src/styles/7-utilities/sr-only.css` 참조.
...
**왜 유용한가:** 시각 사용자는 토스트 메시지나 배지 카운트 변경으로 상태를 파악한다. 스크린리더 사용자에게는 `aria-live` 영역에 텍스트를 삽입하여 알려줄 수 있다. `.sr-only`로 숨기면 시각적으로는 별도 토스트가 표시되고, 스크린리더에는 즉시 텍스트가 읽힌다.
...
**왜 필요한가:** CSS `::after`로 추가된 `*` 기호는 스크린리더가 읽을 수도, 읽지 않을 수도 있다. `.sr-only`로 명시적 안내를 제공하면 확실하다. 다만, `required` 속성만으로도 스크린리더가 "필수"를 알리므로 보완적 역할이다.
...
**왜 필요한가:** 스크린리더의 링크 목록 기능으로 탐색할 때, "더보기"가 여러 개면 어떤 "더보기"인지 구분할 수 없다. `.sr-only`로 맥락을 추가하면 "더보기 뉴스 목록", "더보기 이벤트 목록"으로 구분된다.
...
- `aria-hidden="true"`와 `.sr-only`를 같은 요소에 동시 적용하지 않는다 (서로 모순: 숨기면서 읽히게 하는 것은 불가)
- 이미 있는 정보를 `.sr-only`로 중복 추가하지 않는다 (스크린리더 사용자에게 같은 내용이 두 번 읽힌다)
- 대량의 텍스트를 `.sr-only`로 숨기지 않는다 (스크린리더 사용자에게 예상치 못한 긴 텍스트가 읽힌다)

## CSS 구현

`src/styles/7-utilities/sr-only.css`에 구현되어 있다.
...
| 속성 | 역할 |
|------|------|
| `position: absolute` | 문서 흐름에서 제거, 레이아웃에 영향 없음 |
| `width: 1px; height: 1px` | 최소 크기로 축소 (0px이면 일부 스크린리더가 무시) |
| `margin: -1px` | 1px 공간도 시각적으로 상쇄 |
| `overflow: hidden` | 1px 영역 밖 콘텐츠 숨김 |
| `clip: rect(0, 0, 0, 0)` | 보이는 영역을 0으로 클리핑 (레거시 브라우저 지원) |
| `white-space: nowrap` | 텍스트 줄바꿈 방지 (1px 너비에서 불필요한 줄바꿈 차단) |
| `border: 0` | 보더에 의한 시각적 표시 제거 |
