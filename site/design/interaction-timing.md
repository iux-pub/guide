...
개발 전달을 위한 인터랙션 명세. 모든 전환 효과는 가능한 한 토큰 기반으로 정의하며, 접근성(prefers-reduced-motion)을 필수로 대응한다.
...
Hover 상태는 디자인 전달 문서와 코드 상태 클래스에서 동일하게 확인 가능해야 한다.
...
**전달 시 반드시 명시**: 디자인 전달 문서에 `prefers-reduced-motion` 대응이 필수임을 기재한다.
