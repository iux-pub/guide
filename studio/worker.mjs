...
// 한 번에 몇 개까지 동시에 그릴까 — **하나다.**
//
// 동시에 돌리면 되레 느리다. 같은 프롬프트로 잰 값(2026-08-24):
//   둘 차례로   23초
//   둘 동시     38초
// claude 프로세스는 겉보기와 달리 가볍지 않아 서로 방해한다. 넷을 한꺼번에 던졌을 때
// 「넷 다 시간 초과」가 났던 것도 같은 이유다(「인포마인드 회사 로고」).
// 하나씩 돌리면 각자 제 속도를 내고 합쳐도 더 빠르다.
