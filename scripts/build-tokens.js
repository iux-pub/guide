...
// ── 6. prompts/design.md 토큰 섹션 갱신 ──────────────

function buildPromptTokens() {
  const promptPath = path.join(ROOT, 'prompts', 'design.md')
  if (!fs.existsSync(promptPath)) {
    console.log('  prompts/design.md 없음 — 토큰 섹션 생성 건너뜀')
    return
  }
  ...
  if (startIdx === -1 || endIdx === -1) {
    console.log('  prompts/design.md에 TOKEN_START/TOKEN_END 마커 없음 — 건너뜀')
    return
  }
  ...
  console.log('  prompts/design.md 토큰 섹션 갱신 완료')
}

/**
 * tokens.json 기반 마크다운 토큰 문서 생성
 */
function generateTokenDoc() {
  ...
  if (tokens.typography) {
    for (const [group, entries] of Object.entries(tokens.typography)) {
      if (group.startsWith('$')) continue
      for (const [name, token] of Object.entries(entries)) {
        if (typeof token !== 'object' || !token.$value) continue
        const prefix = group === 'leading' ? 'leading' : `${group}`
        lines.push(`| \`--${prefix}-${name}\` | \`${token.$value}\` |`)
      }
    }
  }
  ...
  lines.push('### 그리드 토큰')
  lines.push('')
  lines.push('| 토큰 | 모바일 | 태블릿 | PC |')
  lines.push('|------|--------|--------|-----|')
  if (tokens.grid) {
    const g = tokens.grid
    lines.push(`| \`--grid-columns\` | ${g.columns.mobile.$value} | ${g.columns.tablet.$value} | ${g.columns.tablet.$value} |`)
    lines.push(`| \`--grid-gutter\` | ${g.gutter.mobile.$value} | ${g.gutter.tablet.$value} | ${g.gutter.tablet.$value} |`)
    lines.push(`| \`--grid-margin\` | ${g.margin.mobile.$value} | ${g.margin.tablet.$value} | ${g.margin.pc.$value} |`)
    lines.push(`| \`--container-max-width\` | ${g['container-max-width'].mobile.$value} | ${g['container-max-width'].mobile.$value} | ${g['container-max-width'].pc.$value} |`)
  }
  ...
  lines.push('| 토큰 | 값 |')
  lines.push('|------|-----|')
  const miscGroups = { radius: 'radius', shadow: 'shadow', transition: 'transition', 'z-index': 'z' }
  for (const [group, prefix] of Object.entries(miscGroups)) {
    if (tokens[group]) {
