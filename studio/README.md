...
## 어떤 주제가 잘 나오나 (2026-08-23 실측 6회)

**단순한 형태일수록 잘 나온다.** 쇼핑백·문서·화살표처럼 윤곽이 하나인 것은 바로 쓸 만하다.

**내부에 격자나 점이 많은 주제(QR 코드, 표, 캘린더 격자)는 잘 안 된다.**
바깥 윤곽을 한 겹만 그려서 「까만 몸통에 흰 구멍」이 되는 일이 반복된다.
프롬프트를 네 번 고치고 자동 재시도까지 넣었지만 이 주제군은 개선되지 않았다.

그래서 **자동 통과가 없다.** 검사가 「선이 많이 굵습니다」·「속이 꽉 찬 덩어리입니다」로
알려 주고, 덩어리는 아예 고를 수 없게 막는다. 나머지는 눈으로 보고 정한다.

복잡한 아이콘이 필요하면 두 가지 길이 있다.

1. 요청을 쪼갠다 — 「QR 코드」만 따로 만들고 「티켓」을 따로 만들어 조합한다
2. 나온 것을 뼈대로 삼아 Figma·일러스트레이터에서 다듬은 뒤 넣는다
