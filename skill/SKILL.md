...
1. ✅ 사용하려는 색상은 `--color-*` 시맨틱 토큰인가?
2. ✅ 폰트는 `--font-sans` 또는 `--font-mono` 기준인가?
3. ✅ CSS nesting과 `@apply`를 쓰더라도 결과 구조가 읽기 쉬운가?
...
- 옛 토큰명 (`--color-text-secondary`, `--font-size-base`, `--spacing-md`, `--radius-base`, `--transition-fast`, `--leading-tight`) → 현행 토큰 또는 프로젝트 규칙으로 교체 권장
