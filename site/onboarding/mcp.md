...
발주처·협력사처럼 MCP를 붙이기 어려운 환경에는 텍스트 문서를 그대로 준다. 섹션마다 요약과 전문 두 형태가 있다.
...
예: <https://footer.kr/guide/_site/components/llms-full.txt> 를 대화에 붙이면 컴포넌트 기준 전체가 전달된다.

작업을 계속 맡길 거라면 MCP가 낫다. 필요한 부분만 도구로 가져가므로 대화가 짧아지고, 기준이 바뀌어도 자동으로 반영된다.
