...
| 컴포넌트 root 태그 `html-semantics.md` 매핑과 불일치 (예: `<div class="card">`, `<div class="modal">`) | R-15 — 시맨틱 의미 손실 |
| 인터랙티브 컴포넌트(modal/tab/accordion/tooltip/disclosure/carousel/calendar)에서 필수 ARIA 속성 누락 | R-16 — 스크린리더 접근 불가 |
| 비-BEM 상태 클래스 (`.is-active`, `.has-error`, `.is-open` 등) | R-17 — BEM modifier로만 표현 |
| 시각적 단어 modifier (`--blue`, `--red`, `--big`, `--rounded`, `--shadow` 등) | R-18 — 의미 기반 modifier만 |
