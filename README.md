...
| 명령어 | 설명 |
|--------|------|
| `npm run build` | 전체 빌드 (tokens → CSS → prompts → site) |
| `npm run build:css` | SCSS 컴파일 |
| `npm run build:tokens` | tokens.json → SCSS 토큰 자동 생성 |
| `npm run build:prompts` | AI 프롬프트 자동 재생성 |
| `npm run build:site` | 문서 사이트 빌드 |
| `npm run watch:css` | SCSS 워치 모드 |
| `npm run serve` | 문서 사이트 개발 서버 (CSS 핫리로드) |
| `npm run lint:css` | Stylelint BEM 검사 |
| `npm run lint:css:fix` | 자동 수정 |
| `npm run test` | 전체 검증 (lint + build + a11y) |
| `npm run test:a11y` | pa11y-ci 접근성 테스트 |
| `npm run sync:starter` | starter 저장소 동기화 (SCSS/JS 변경 후 수동 실행) |
