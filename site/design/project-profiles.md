...
KRDS 체크리스트는 모든 프로젝트에 같은 방식으로 적용하지 않는다. `infoUX`는 먼저 사이트 유형을 판정한 뒤, **공통 생성 규칙**과 **조건부 정부/공공 규칙**을 분리해 적용한다.
...
1. 사이트 유형을 먼저 판정한다.
2. 유형이 불명확하면 일반사이트 또는 CMS·관리자로 가정하고 정부 아이덴티티 요소는 생성하지 않는다.
