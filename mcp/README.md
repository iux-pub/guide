...
`info-design` 스킬은 Claude Code 전용이고 `~/.claude/skills/`에 직접 복사해야 한다. MCP는 등록 한 줄로 끝나고 Codex·Cursor에서도 똑같이 붙는다.
...
`infoUX MCP 준비됨 — 빌드 <sha>, 도구 7종`이 stderr로 나오면 정상이다. stdout은 프로토콜 채널이라 로그를 싣지 않는다.
